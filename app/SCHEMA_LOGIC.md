...
từ `app/infrastructure/database/schemas/`. Đây là nguồn tham chiếu cho code mới;
các SQL trong `backend/` chỉ phục vụ subsystem kế thừa và không phải schema của
workspace desktop.
...
và không dùng schema kế thừa trong `backend/` để thay thế schema desktop.
