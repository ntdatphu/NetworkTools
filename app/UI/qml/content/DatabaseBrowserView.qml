...
Rectangle {
    ...
    function saveCell(rowData, columnName, value) {
        ...
        if (result.ok) {
            ...
            updated.message = result.message || "Update failed."
            tableData = updated
        }
    }
    ...
    ColumnLayout {
        ...
        Rectangle {
            ...
            ScrollView {
                ...
                Column {
                    width: Math.max(tableRows.width, (root.tableData.columns || []).length * 160)
                    ...
                    ListView {
                        id: tableRows
                        width: Math.max(parent.width, (root.tableData.columns || []).length * 160)
                        height: Math.max(0, root.height - 96)
                        clip: true
