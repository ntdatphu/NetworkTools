...
Popup {
    ...
    function openPreview() {
        ...
        if (host === "") {
            notify("Select a device tab before previewing DHCP push.", "warning")
            return
        }

        const payload = dbManager.previewDhcpConfig(host)
        ...
        if (!ok) {
            notify(messageText || "Cannot preview DHCP configuration.", "error")
            return
        }
    ...
    function pushNow() {
        ...
        isPushing = true
        const result = dbManager.pushDhcpConfig(host)
        isPushing = false

        const ok = result && (result.ok === undefined || result.ok === true)
        const msg = result && result.message ? String(result.message) : (ok ? "DHCP push completed." : "DHCP push failed.")
        messageText = msg
    ...
    contentItem: ColumnLayout {
        ...
        Rectangle {
            ...
            TextArea {
                anchors.fill: parent
                anchors.margins: 10
                text: dialog.previewText === "" ? "No pending DHCP configuration to push." : dialog.previewText
                readOnly: true
                selectByMouse: true
