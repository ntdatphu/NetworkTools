...
Item {
    ...
    DeviceContextMenu {
        ...
        onUpDevRequested: (ip) => { const okDev = dbManager.updateDeviceDev(ip, 1); const okSuccess = dbManager.updateDeviceSuccess(ip, 1); const ok = okDev && okSuccess; if (typeof statusBar !== "undefined") statusBar.showMessage(ok ? "Dev enabled for " + ip + "." : "Failed to enable dev for " + ip + ".", ok ? "success" : "error"); if (ok) devicesPanel.reloadDevices() }
        onDownDevRequested: (ip) => { const okDev = dbManager.updateDeviceDev(ip, 0); const okSuccess = dbManager.updateDeviceSuccess(ip, 0); const ok = okDev && okSuccess; if (typeof statusBar !== "undefined") statusBar.showMessage(ok ? "Dev disabled for " + ip + "." : "Failed to disable dev for " + ip + ".", ok ? "success" : "error"); if (ok) devicesPanel.reloadDevices() }
