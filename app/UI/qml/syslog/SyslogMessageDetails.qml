...
Dialog {
    ...
    ColumnLayout {
        ...
        Text { text: "Received: " + (root.rowData.received_at || "-"); color: Theme.textSecondary }
        Text { text: "Device time: " + (root.rowData.device_time || "-"); color: Theme.textSecondary }
        Text { text: "Facility/Severity: %1/%2".arg(root.rowData.facility || "-").arg(root.rowData.severity); color: Theme.textSecondary }
        Text { text: "Parse status: " + (root.rowData.parse_status || "-"); color: Theme.textSecondary }
        TextArea {
