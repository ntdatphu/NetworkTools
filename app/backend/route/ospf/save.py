...
def _sync_ospf_networks(conn: sqlite3.Connection, db: Any, ospf_id: int, process: dict[str, Any]) -> None:
...
def _load_process_for_compare(conn: sqlite3.Connection, db: Any, ospf_id: int) -> dict[str, Any] | None:
...
def _reset_ospf_process_children(conn: sqlite3.Connection, ospf_id: int) -> None:
    for table in (
        "ospf_networks",
        "ospf_distance",
        "ospf_areas",
...
def _archive_ospf_process(conn: sqlite3.Connection, ospf_id: int) -> None:
    conn.execute("UPDATE ospf_processes SET success = -1 WHERE ospf_id = ?;", (ospf_id,))
    _reset_ospf_process_children(conn, ospf_id)


def _is_blank_ospf_process_submission(db: Any, process: dict[str, Any]) -> bool:
...
def _describe_process_submission(db: Any, process: dict[str, Any], index: int) -> str:
    return (
        f"submission #{index}: "
...
def _insert_ospf_process(conn: sqlite3.Connection, db: Any, host: str, process: dict[str, Any]) -> int:
    ...
    try:
        with db._connect() as conn:
            ...
            for index, process_value in enumerate(db._as_list(payload), start=1):
                ...
                if _is_blank_ospf_process_submission(db, process):
                    continue
                ...
                if process_id is None:
                    raise ValueError(
                        ...
                        + _describe_process_submission(db, process, index)
                    )

                if ospf_id > 0 and ospf_id in existing_ids:
                    submitted_ids.add(ospf_id)
                    current = _load_process_for_compare(conn, db, ospf_id)
                    ...
                    if (
                        ...
                        _sync_ospf_networks(conn, db, ospf_id, process)
                        continue

                    _archive_ospf_process(conn, ospf_id)
                    _insert_ospf_process(conn, db, host, process)
                    continue

                _insert_ospf_process(conn, db, host, process)

            for deleted_id in existing_ids - submitted_ids:
                _archive_ospf_process(conn, deleted_id)
