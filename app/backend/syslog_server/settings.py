"""Persistent and validated QSettings facade exposed to QML."""
...
def _local_ipv4_addresses() -> list[str]:
    """Return usable IPv4 addresses from active local network interfaces."""
    try:
    ...
    def validate(self) -> dict[str, object]:
        ...
        try:
            ...
            if self.advertisedIp not in self._available_advertised_ips:
                raise ValueError("Advertised/server IP must be assigned to an active network interface on this machine")
        except ValueError as exc:
            return {"ok": False, "message": str(exc)}
        return {"ok": True, "message": "Syslog settings are valid."}

    def listener_config(self) -> ListenerConfig:
        # Receiving packets does not require an advertised IP. That address is
        # required only when pushing a destination to a network device.
