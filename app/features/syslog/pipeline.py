"""Atomic lifecycle for the Syslog socket receiver and SQLite writer."""
...
class SyslogPipeline:
    """Start/stop receiver and writer as one rollback-safe unit."""
    ...
    def start(self, config: ListenerConfig) -> SyslogReceiver:
        with self._lock:
            ...
            receiver = self._receiver_factory(
                config, self.writer.submit, self._on_receiver_error
            )
            try:
                receiver.start()
            except Exception:
