...
- Đổi `acl_name` hoặc `acl_type`:
  mark NAT ACL cũ `success = -1`, insert NAT ACL mới `success = 0`.
- Đổi `description`:
...
- Hầu hết bảng L2 không dùng `success`.
- `svi_interface` có `success` để đánh dấu cấu hình SVI.
...
| File | Luồng | Ghi chú |
|------|-------|---------|
| `OspfRoutingRepository.cpp` | C++ -> DB | Logic phải khớp với schema OSPF hiện tại |
...
- Với SQLite, `UNIQUE` trên cột nullable vẫn cho phép nhiều row có `NULL`; nếu cần chống trùng logic tuyệt đối, nên chuẩn hóa giá trị `NULL` ở tầng ứng dụng.
