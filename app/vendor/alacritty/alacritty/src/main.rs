...
fn alacritty(mut options: Options) -> Result<(), Box<dyn Error>> {
    // Keep managed windows visible when OpenSSH exits so the user can read the
    // negotiation/authentication error instead of seeing a window flash closed.
    if options.nt_managed {
        options.window_options.terminal_options.hold = true;
    }
    ...
    let managed_options = if options.nt_managed {
        Some(networktools::ManagedOptions {
            ...
            host: options.nt_host.clone().ok_or("--nt-host is required")?,
            ipc_path: options.nt_ipc.clone().ok_or("--nt-ipc is required")?,
        })
    } else {
        None
    };
    ...
    #[cfg(unix)]
    if let Some(client) = &nttp_client {
        client.event("terminal.ready", serde_json::json!({}));
        client.event("child.started", serde_json::json!({}));
    }
