...
def main():
    ...
    try:
        ...
        for host in target_hosts:
            ...
            for n_id, n_name, n_type, n_desc, n_success in cursor.fetchall():
                ...
                for iid, iname, irole, isuccess in cursor.fetchall():
                    istate = "remove" if isuccess == -1 else "setup"
                ...
                for pid, pname, start, end, mask, prefix, psuccess in cursor.fetchall():
                    pstate = "remove" if psuccess == -1 else "setup"
                ...
                for sid, l_ip, g_ip, proto, l_port, g_port, ext, ssuccess in cursor.fetchall():
                    sstate = "remove" if ssuccess == -1 else "setup"
                ...
                # Dynamic Rules (JOIN to get names)
                cursor.execute(f"""
                ...
                for oid, acl_n, out_intf, ovl, osuccess in cursor.fetchall():
                    ostate = "remove" if osuccess == -1 else "setup"
        ...
        run_nat_config(valid_data, DB_PATH, ROUTE_OUTPUT) # Reusing ROUTE_OUTPUT for logging simplicity
                ...
                if res.get("status") == "success":
                    ...
                    for item in valid_data:
                        if item["target"]["ip"] == ip:
                            # Update ACLs
                            for aid in item["tracking"]["acl_add"]: cursor.execute(f"UPDATE {T_NAT_ACL_MAIN} SET success = 1 WHERE nat_acl_id = ?", (aid,))
                            for aid in item["tracking"]["acl_del"]: cursor.execute(f"DELETE FROM {T_NAT_ACL_MAIN} WHERE nat_acl_id = ?", (aid,))
                            # Update NATs
                            for nid in item["tracking"]["nat_add"]: cursor.execute(f"UPDATE {T_NAT_MAIN} SET success = 1 WHERE nat_id = ?", (nid,))
