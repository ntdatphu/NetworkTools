...
def task_push_vtp(task):
    # Lấy payload đã được tiêm qua Inventory
        ...
        for item in task_list:
            ...
            if row:
                ...
                hosts_yaml[dev_name or ip] = {
                    ...
                    "data": {"vtp_payload": item["vtp_data"]}  # Tiêm cấu hình VTP
                }
        conn.close()
...
def run_vtp_worker(input_data, db_path, output_path):
    ...
    # Khởi tạo Nornir bằng Config Dict, không xài YAML tĩnh
    nr = InitNornir(
    ...
    # Ghi log chuẩn để main.py nhận diện
