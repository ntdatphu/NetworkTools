...
def normalize_interface_name(name: str) -> str:
    """Phiên dịch tên cổng viết tắt thành tên đầy đủ để khớp với Database"""
    if not name: return ""
    name = name.strip()
...
def sync_l2_interface_worker(host_ip: str):
    """Đọc file text _l2_interfaces.txt và đồng bộ vào DB"""
    file_path = os.path.join(L2_BACKUP_DIR, host_ip,f"{host_ip}_l2_ifaces.txt")
    ...
    # --- 1. BĂM DỮ LIỆU BẰNG REGEX ---
    intf_status, intf_trunk, etherchannel = [], [], []

    # Regex bắt show interfaces status
    ...
    for m in status_pattern.finditer(content):
        intf_status.append({
    ...
    # --- 2. ĐỔ VÀO DATABASE (Giữ nguyên logic cũ của sếp) ---
    conn = get_db_connection()
    c = conn.cursor()
    try:
        # Đồng bộ Etherchannel
