...
Cập nhật: **2026-08-16**. `backend/` và `api_server.py` là subsystem nghiên cứu
kế thừa, **không** được `app/main.py` khởi tạo và không dùng database workspace
đang mở của desktop.
...
đối chiếu khi chuyển tính năng sang `app/features`, nhưng đường import, cấu hình,
...
- SQL trong `backend/sql/` và `backend/PyCode/share/database/` là schema kế thừa,
  không thay thế schema canonical ở `app/infrastructure/database/schemas/`;
