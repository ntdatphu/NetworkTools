...
# =====================================================================
# THÊM IMPORT CHO CUSTOM INVENTORY PLUGIN NORNIR 3+
# =====================================================================
from nornir.core.inventory import Inventory, Host
from nornir.core.plugins.inventory import InventoryPluginRegister

# =====================================================================
# KẾT NỐI VỚI CONFIG TRUNG TÂM (Tự động mò tìm thư mục gốc)
# =====================================================================
...
# =====================================================================
# ĐỊNH NGHĨA CUSTOM PLUGIN ĐỂ NORNIR ĐỌC TỪ DICTIONARY DB
# =====================================================================
...
# ==========================================
# 1. HÀM PROBE THIẾT BỊ (NORNIR TASK)
# ==========================================
def login_and_probe_task(task: Task):
    ...
    if method == "RESTCONF":
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                if res_host.status_code == 200:
                    try:
                        real_hostname = res_host.json().get("Cisco-IOS-XE-native:hostname", host_ip)
                    except: pass
            elif res.status_code == 401:
                error_msg = "Sai tài khoản/mật khẩu RESTCONF"
            else:
                error_msg = f"RESTCONF từ chối truy cập (Mã: {res.status_code})"
        except requests.exceptions.Timeout:
            error_msg = "Timeout: Không thể kết nối RESTCONF (Kiểm tra IP/Port)"
        except Exception as e:
            error_msg = f"Lỗi RESTCONF: {str(e)}"

    elif method == "NETCONF":
        try:
            with manager.connect(
                host=host_ip, port=port, username=user, password=pw,
                hostkey_verify=False, device_params={'name': 'default'}, timeout=10
            ) as m:
        ...
        # =====================================================================
        # KHU VỰC XỬ LÝ SSH / TELNET - ĐÃ TÍCH HỢP ĐỦ ROLE XỊN
        # =====================================================================
        candidates = ["cisco_ios", "mikrotik_routeros", "huawei_vrp"]
        if method == "TELNET": 
            candidates = ["cisco_ios_telnet", "huawei_vrp_telnet"]
        ...
        for os_type in candidates:
            try:
                ...
                with ConnectHandler(**device_dict) as net_connect:
                    ...
                    # Rút thông tin Version bản gốc và bản IN HOA
                    sh_ver_raw = net_connect.send_command("show version")
                    sh_ver = sh_ver_raw.upper()

                    # ========================================================
                    # 2. BỘ NHẬN DIỆN ROLE THÔNG MINH (Đã Fix lỗi Hoa/Thường)
                    # ========================================================
                    try:
                        sh_run_routing = net_connect.send_command("show run | include ip routing")

                        if any(kw in sh_ver for kw in ["ASA", "FIREPOWER", "PIX"]):
                            role = "Firewall"
                            
                        elif any(kw in sh_ver for kw in ["AIR-", "WIRELESS", "WLC"]):
                            role = "Access Point"
                            
                        elif any(kw in sh_ver for kw in ["VIOS_L2", "CATALYST", "C2960", "C3560", "C3850", "C9300"]):
                            if "ip routing" in sh_run_routing:
                                role = "Switch L3"
                            else:
                                role = "Switch L2"
                                
                        elif any(kw in sh_ver for kw in ["IOSV", "VIOS", "ISR", "ASR", "C19", "C29"]):
                            role = "Router"
                            
                        elif any(kw in sh_ver for kw in ["LINUX", "UBUNTU", "CENTOS"]):
                            role = "Server"
                ...
                error_msg = "Sai tài khoản/mật khẩu đăng nhập"
                break 
            except NetmikoTimeoutException:
                error_msg = "Timeout: Không thấy thiết bị phản hồi"
                break
            except Exception:
                continue 

        if not connection_success and not error_msg:
            error_msg = "Thiết bị từ chối kết nối hoặc OS không hỗ trợ"

    task.host.data.update({
        "real_name": real_hostname, 
        "os": detected_os, 
        "role": role, 
        "success": success,
        "error_msg": error_msg
...
# ==========================================
# 2. HÀM MAIN: DB -> NORNIR -> JSON
# ==========================================
def main():
    db_path = DB_PATH
    json_export_path = os.path.join(TMP_DIR, "login_output.json")
    ...
    try:
        ...
        cursor = conn.cursor()
        cursor.execute("SELECT host, method, portnumber, username, password FROM devices WHERE success = 0 OR success IS NULL")
    ...
    for ip, task_result in results.items():
        ...
        report_item = {
            ...
            "method": host_data.get("method"),
            "hostname": host_data.get("real_name", ""),
            "os": host_data.get("os", ""),
            "role": host_data.get("role", ""),
            "error_reason": host_data.get("error_msg", "")
        }
        ...
        if is_success:
            ...
            cursor.execute("DELETE FROM devices WHERE host=?", (ip,))
