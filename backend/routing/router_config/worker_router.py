...
def main():
    ...
    try:
        ...
        write_report("error", "Unknown", "router", "Lỗi đọc file cấu hình YAML", e)
        return

    # --- BƯỚC 2: CHỌN TEMPLATE ---
    template_name = 'template_router.j2'
    if action == 'delete':
        template_name = 'template_router_delete.j2'
        print(f"  CẢNH BÁO: Đang thực hiện chế độ XÓA (DELETE) trên Router {target_ip}")
    ...
    try:
        ...
        write_report("error", target_ip, "router", f"Lỗi xử lý Template {template_name}", e)
        return
    ...
    device = driver(
        ...
        data['username'], 
        data['password'],
        # [FIX 1] Tăng thời gian chờ lên gấp 4 lần để Router kịp thở
        optional_args={
    ...
    try:
        print(f"   -> Đang kết nối tới Router {target_ip}...")
        device.open()
        ...
        # [FIX 2] Tắt log console trước khi chạy lệnh xóa để tránh nhiễu
        final_commands = ['no logging console'] + clean_commands

        # [FIX 3] Tăng timeout lên 60s cho chắc ăn
        ...
        print(f"\n[ROUTER] Thực thi lệnh '{action.upper()}' thành công!")

        # GHI BÁO CÁO THÀNH CÔNG
        msg = "Cấu hình thành công!" if action == 'configure' else "Đã xóa cấu hình thành công!"
        write_report("success", target_ip, "router", msg, output)
        
    except Exception as e:
        print(f" Lỗi thực thi Router: {e}")
        write_report("error", target_ip, "router", "Lỗi kết nối hoặc đẩy lệnh", e)
