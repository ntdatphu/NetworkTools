...
- Tổng cộng 27 bảng.
- Mô hình theo kiểu parent-child rõ ràng:
...
- Routing:
  - ROUTING_DB là parent theo route_type và host.
  - static_default_routes, static_routes cho static/default route.
  - static_default_routes, static_routes có thêm cột `success` để theo dõi trạng thái bản ghi (0: active, -1: soft-delete).
...
- Routing:
  - ROUTING_DB as parent by route_type and host.
  - static_default_routes, static_routes for default/static routing.
  - static_default_routes, static_routes now include `success` to track row state (0: active, -1: soft-deleted).
