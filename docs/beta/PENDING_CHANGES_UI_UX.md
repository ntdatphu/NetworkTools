...
| UX-09 | PARTIAL | Icon cho action button: chỉ gắn khi có asset chuyên biệt đúng nghĩa. Add/New, Cancel, Logs controls và Tool Catalog utility giữ text-only để tránh lỗi bố cục hoặc dùng icon gần nghĩa. Bộ 44 SVG SFTP đã được phục hồi và các action SFTP phù hợp đã dùng icon; hiện 111/170 `StandardButton` không khai báo icon, được kiểm kê ở mục P2. |
...
- [x] Hoàn thiện vòng thiết kế thứ hai cho cả Interfaces/Switching/Security/Monitoring: summary theo ngữ cảnh, toolbar tìm kiếm, table schema đúng tác vụ và inspector progressive disclosure; Security không còn tạo port, Monitoring format byte và tách error/discard rõ ràng;
- [x] `SwitchWorkspace` incubation bất đồng bộ và cache từng trang sau lần mở đầu; chuyển Feature giữ draft/selection và truyền trạng thái loading tới icon Device Tab;
- [x] Kiểm tra trực quan trên bản chạy riêng ở light theme cho Interfaces, VLAN, Port Security, Port Counters và edit state; chưa thay đổi dữ liệu thiết bị trong lượt kiểm tra.

Chi tiết audit, quyết định thiết kế, phạm vi áp dụng/loại trừ và backlog còn lại:
[`TABLE_UI_FAMILY.md`](TABLE_UI_FAMILY.md) và
...
Phạm vi kiểm kê là toàn bộ file QML dưới `app/UI/`; `ContextMenuItem`, Activity Bar item và component không phải `StandardButton` không nằm trong mẫu số. Kết quả hiện tại: **170 nút, 59 có icon binding, 111 không khai báo icon**. Security không còn action Add vì policy chỉ áp dụng cho port đã tồn tại. Switching đã thay hàng button tự dựng bằng `SubBar` dùng chung để đồng nhất SubFeatureBar. SFTP đã nhận lại 44 SVG cùng hai README giấy phép từ nhánh nguồn; bảy button action mới dùng icon đúng ngữ nghĩa, còn New Folder giữ text-only theo quy tắc Add/New. `SftpLogPanel` cũng được phục hồi với giới hạn 500 sự kiện để không tăng RAM vô hạn. Bảy nút Logs/Tool Catalog giữ text-only do chưa có asset chuyên biệt đúng nghĩa. `ConfigTextViewer` có hai nút chevron, ba nút zoom glyph/text; hai consumer thêm Copy All dùng `clipboard-copy.svg`. Binding động chỉ hiện icon khi action mang nghĩa Save. Contract test giữ các con số này đồng bộ với code; khi thêm/bớt nút phải cập nhật bảng và test cùng thay đổi.
...
- [x] Full suite desktop đạt 128/128 ngày 2026-07-18; QML smoke không warning, gồm database bootstrap/schema parity, asset/QML export, loader lifecycle, Switch responsive/cache/contextual inspector + pre-merge compatibility, họ table, bảng DHCP/NAT và routing networks, SFTP asset/session log, Device Logs, Tool Catalog, Feature Bar CLI và External Tools.
