...
Đề cương này được đối chiếu với mã nguồn trong `app/` tại thời điểm cập nhật. Khi viết báo cáo chính thức phải phân biệt ba mức:
...
Entry point là `app/main.py`, nạp QML module `UI`. Runtime chính không phải kiến trúc web client/server.
...
| ACL | Đã có UI và persistence | Standard, Extended, Dynamic, Reflexive, MAC ACL; rule và binding interface | Chưa có View & Push controller trong `app/` |
...
Mô tả `app/main.py`, các context property, QML module `UI`, settings và network monitor.
...
Trình bày UI/persistence các loại ACL, rule có thứ tự và binding nhiều interface. Ghi rõ chưa có controller sinh lệnh/push trong `app/`.
