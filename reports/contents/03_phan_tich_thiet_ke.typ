...
#[
  ...
  #report-table(
    ...
    rows: (
      ([`t01_*`], [Hệ thống thiết bị cốt lõi], [Lưu trữ định danh thiết bị, thông tin xác thực, thuật toán SSH và trạng thái kết nối.]),
      ([`t02_*`], [Giao diện mạng (Interface)], [Quản lý thông số IPv4, Subinterface, Tunnel và các tham số vật lý của Router.]),
      ([`t03_*`], [Dịch vụ IP (DHCP & Helper)], [Lưu trữ cấu hình cấp phát IP động, ngoại trừ (excluded IP) và DHCP Relay.]),
      ([`t04_*`], [Định tuyến (Routing)], [Quản lý bảng định tuyến tĩnh, các tiến trình OSPF, EIGRP và tùy chỉnh trên từng cổng.]),
      ([`t05_*`], [Bảo mật & Biên dịch địa chỉ], [Lưu trữ quy tắc danh sách truy cập (ACL), Route-map và cấu hình NAT/PAT.]),
      ([`t06_*`, `t07_*`], [Chuyển mạch L2 & VRF], [Định nghĩa cấu trúc cho VLAN, STP, EtherChannel và mạng ảo (dự phòng mở rộng).]),
    ),
